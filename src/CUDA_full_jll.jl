...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import CUDA_full_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("CUDA_full_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "CUDA_full_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"CUDA_full", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "CUDA_full_jll", "src", "CUDA_full_jll.jl"))
        @info("CUDA_full_ll dev'ed out to /depot/dev/CUDA_full_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("4f82f1eb-248c-5f56-a42e-99106d144614"))
platforms = [Pkg.Artifacts.unpack_platform(e, "CUDA_full", artifacts_toml) for e in artifacts["CUDA_full"]]
...
if best_platform === nothing
    @debug("Unable to load CUDA_full; unsupported platform $(triplet(platform_key_abi()))")
else
